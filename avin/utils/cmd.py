...
class Cmd:
    @staticmethod
    def path(*path_parts) -> Path:
        """Склеивает отдельные строки в путь"""
    ...
    def name(file_path: Path, extension: bool = False) -> str:
        ...
        /home/user/file_name.txt -> file_name.txt  # extension=True
        /home/user/file_name.txt -> file_name  # extension=False
        """
    ...
    def dir_path(file_path: Path) -> Path:
        assert Cmd.is_file(Path(file_path))
    ...
    @staticmethod
    def content(dir_path: str, full_path=False) -> list[Path]:
    ...
    def select(files: list[str], name=None, extension=None) -> list[str]:
        ...
        selected = list()
        for file in files:
            if name is not None and name == Cmd.name(Path(file)):
