...
async def test_info(event_loop):
    ...
    info_list = await Keeper.info(
        DataSource.TINKOFF, Instrument.Type.SHARE, ticker="SBER"
    )
    ...
    info_list = await Keeper.info(DataSource.TINKOFF, Instrument.Type.SHARE)
...
async def test_instrument_id(event_loop):
    ...
    # get instrument id
    ids = await Keeper.get(Instrument, figi="bbyby")
    assert len(ids) == 1
